...
int main()
{
	...
	int iPI1 = PI;	//暗黙的変換で、エラーが出ないが、warning C4244が出る
	int iPI2 = static_cast<int>( PI );		//C++的キャストの書き方。丁寧
	int iPI3 = int(PI);						//C++的キャストの書き方
	int iPI4 = (int)PI;						//C言語的キャストの書き方

	//dymanic_cast
	//(基底クラスから派生クラスへキャストダウンする際に、型を厳密にチェックする方法)
	//dymanic_cast<変換する型名>(変換元の値(変数))
	Enemy01 enemy01;
	CharaterBase* pBase = &enemy01;		//基底のポインタ変数へ、派生のアドレス代入
...
//	int* pNonConst = &iConst	//constでないポインタ変数へアドレスを渡すこともできない
	...
	*pNonConst = 200;
	...
	//*pNonConst = 400; //グローバル変数の変数の場合例外がスローされる
